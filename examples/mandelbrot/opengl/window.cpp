...
vec2 scaledFragCoord = (vec2(fragCoord) - viewportSize / 2.0) / viewportSize;
float real = scaledFragCoord.x;
float imag = scaledFragCoord.y;
...
void main() { 
  // float real = (2.0 * (gl_FragCoord.x / viewportSize.x) - 1.0);
  // float imag = (2.0 * (gl_FragCoord.y / viewportSize.y) - 1.0);
  // float vwx = float(viewportSize.x);
  // float vwy = float(viewportSize.y);
  // frag_color = vec4(vwx/600.0,viewportSize.y/600,0,1);
  ...
  viewportSizeLocation = glGetUniformLocation(m_program, "viewportSize");
  constant_real_part =
...
void Window::onPaint() {
  ...
  glUniform2f(viewportSizeLocation, m_viewportSize.x, m_viewportSize.y);
...
void Window::onDestroy() {
  // libera espaço de memoria que as variaveis ocupavam
  abcg::glDeleteProgram(m_program);
  abcg::glDeleteBuffers(1, &m_VBOVertices);
  abcg::glDeleteVertexArrays(1, &m_VAO);
  // abcg::glDeleteVertexArrays(1, &constant_real_part);
  // abcg::glDeleteVertexArrays(1, &constant_imag_part);
  // abcg::glDeleteVertexArrays(1, &max_iter_r);
  // abcg::glDeleteVertexArrays(1, &max_iter_g);
  // abcg::glDeleteVertexArrays(1, &max_iter_b);
}
